...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("ZerothXMusic.plugins" + all_module)
    LOGGER("ZerothXMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Anony.start()
    try:
        await Anony.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("ZerothXMusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("ZerothXMusic").info("Stopping Zeroth X Music Bot...")
